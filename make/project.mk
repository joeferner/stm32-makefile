...
#
# This makefile requires the environment variable STM32_MAKEFILE_PATH to be set to the top-level STM32 Makefile directory
...
$(warning "stm32-Makefile build system only supports GNU Make versions 3.81 or newer. You may see unexpected results with other Makes.")
endif
endif

# make STM32_MAKEFILE_PATH a "real" absolute path
...
SANITISED_STM32_MAKEFILE_PATH:=$(realpath $(STM32_MAKEFILE_PATH))
else
SANITISED_STM32_MAKEFILE_PATH:=$(realpath $(wildcard $(STM32_MAKEFILE_PATH)))
endif

export STM32_MAKEFILE_PATH := $(SANITISED_STM32_MAKEFILE_PATH)

ifndef STM32_MAKEFILE_PATH
$(error STM32_MAKEFILE_PATH variable is not set to a valid directory.)
endif

ifneq ("$(STM32_MAKEFILE_PATH)","$(SANITISED_STM32_MAKEFILE_PATH)")
# implies STM32_MAKEFILE_PATH was overriden on make command line.
...
# make STM32_MAKEFILE_PATH=~/blah not make STM32_MAKEFILE_PATH="~/blah"
$(error If STM32_MAKEFILE_PATH is overriden on command line, it must be an absolute path with no embedded shell special characters)
endif

ifneq ("$(STM32_MAKEFILE_PATH)","$(subst :,,$(STM32_MAKEFILE_PATH))")
$(error STM32_MAKEFILE_PATH cannot contain colons. If overriding STM32_MAKEFILE_PATH on Windows, use Cygwin-style /c/dir instead of C:/dir)
endif
...
COMMON_MAKEFILES := $(abspath $(STM32_MAKEFILE_PATH)/make/project.mk $(STM32_MAKEFILE_PATH)/make/common.mk $(STM32_MAKEFILE_PATH)/make/component_wrapper.mk)
...
COMPONENT_DIRS ?= $(PROJECT_PATH)/components $(EXTRA_COMPONENT_DIRS) $(STM32_MAKEFILE_PATH)/components
...
STM32_MAKEFILE_VER := $(shell cd ${STM32_MAKEFILE_PATH} && git describe --always --tags --dirty)
...
CPPFLAGS := -DESP_PLATFORM -D STM32_MAKEFILE_VER=\"$(STM32_MAKEFILE_VER)\" -MMD -MP $(CPPFLAGS) $(EXTRA_CPPFLAGS)
...
export CC CXX LD AR OBJCOPY OBJDUMP SIZE STM32CUBEMX
...
include $(STM32_MAKEFILE_PATH)/make/project_config.mk
...
$(APP_BIN): $(APP_ELF)
	$(OBJCOPY) $(OBJCOPYFLAGS) $(APP_ELF) $(APP_BIN)
	$(OBJDUMP) $(OBJDUMPFLAGS) $(APP_ELF) > $(BUILD_DIR_BASE)/main.list
...
+$(MAKE) -C $(BUILD_DIR_BASE)/$(2) -f $(STM32_MAKEFILE_PATH)/make/component_wrapper.mk COMPONENT_MAKEFILE=$(1)/component.mk COMPONENT_NAME=$(2)
endef
...
#
# This only works for components inside STM32_MAKEFILE_PATH
check-submodules:

# Dump the git status for the whole working copy once, then grep it for each submodule. This saves a lot of time on Windows.
GIT_STATUS := $(shell cd ${STM32_MAKEFILE_PATH} && git status --porcelain --ignore-submodules=dirty)

# Generate a target to check this submodule
# $(1) - submodule directory, relative to STM32_MAKEFILE_PATH
define GenerateSubmoduleCheckTarget
check-submodules: $(STM32_MAKEFILE_PATH)/$(1)/.git
$(STM32_MAKEFILE_PATH)/$(1)/.git:
	@echo "WARNING: Missing submodule $(1)..."
	[ -e ${STM32_MAKEFILE_PATH}/.git ] || ( echo "ERROR: STM32 Makefile must be cloned from git to work."; exit 1)
	[ -x $(which git) ] || ( echo "ERROR: Need to run 'git submodule init $(1)' in STM32 Makefile root directory."; exit 1)
	@echo "Attempting 'git submodule update --init $(1)' in STM32 Makefile root directory..."
	cd ${STM32_MAKEFILE_PATH} && git submodule update --init $(1)

# Parse 'git status' output to check if the submodule commit is different to expected
ifneq ("$(filter $(1),$(GIT_STATUS))","")
$$(info WARNING: STM32 Makefile git submodule $(1) may be out of date. Run 'git submodule update' in STM32_MAKEFILE_PATH dir to update.)
endif
endef

# filter/subst in expression ensures all submodule paths begin with $(STM32_MAKEFILE_PATH), and then strips that prefix
# so the argument is suitable for use with 'git submodule' commands
$(foreach submodule,$(subst $(STM32_MAKEFILE_PATH)/,,$(filter $(STM32_MAKEFILE_PATH)/%,$(COMPONENT_SUBMODULES))),$(eval $(call GenerateSubmoduleCheckTarget,$(submodule))))
